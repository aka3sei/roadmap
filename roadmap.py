...
sell_details = {
    "1.売買相談": {
        ...
        "アクション": "・売却希望時期の確定\n・残債の確認（住宅ローン一括返済の可否）\n・住み替え先の内定（先行購入か先行売却か）",
        "重要ポイント": "【戦略の急所】「なぜ売るのか」によって出口戦略が180度変わります。買い替えなら「期限優先」で買取保証を検討し、相続なら「価格優先」で時間をかけて市場に晒します。特に『先行購入』の場合、売却が遅れるとダブルローンのリスクや売却価格の妥協を強いられるため、最初の1ヶ月の反響分析が勝敗を分けます。"
    },
    "2.物件調査": {
    ...
    "9.引渡・登記・決済": {
        ...
        "重要ポイント": "【最後の詰め】決済当日に「権利証がない」「印鑑証明が古い」というミスは許されません。数日前に司法書士と連携し書類を完備します。また、プロは決済の「3日前」に現地の最終確認を推奨します。付帯設備表に書いた通りにエアコンやガスコンロが動作するか、ゴミが残っていないか。最後の最後で「誠実さ」を見せることが、引渡し後の円満な関係と、将来のトラブル防止に直結します。"
    }
}
...
buy_details = {
    "1.購入相談": {
        ...
        "アクション": "・希望エリアの優先順位付け\n・最低限必要なスペックの書き出し\n・将来のライフイベントの整理",
        "ポイント": "【資産価値の視点】自分の好みだけでなく「10年後に誰に売れるか」というリセールバリューの視点をプロは授けます。人口動態、再開発計画、駅徒歩10分以内という『流動性の確保』は、万が一のライフスタイル変更時に家族を救う保険になります。「住みたい家」と「買ってもいい資産」の最大公約数を見つけるのが最初のゴールです。10年後の自分へ送る資産形成としての側面を忘れないでください。"
    },
    "2.資金計画": {
        "内容": "年収から算出する『借りられる額』ではなく、生活費から逆算する『返せる額』を算出。諸費用（物件価格の約7〜10%）を正確に把握します。",
        "アクション": "・金融機関での事前審査（必須）\n・諸費用（仲介手数料、登記費用、印紙代）の内訳確認\n・ペアローン、合算の検討",
        "ポイント": "【真の購買力】金利優遇（0.3%台など）を引き出すには、健康状態や勤務先の属性を活かした銀行選びが不可欠です。また、プロは「物件価格以外」のコストを徹底的に洗い出します。引っ越し代、家具家電、不動産取得税。これらを予備費として確保した上で、事前審査を「満額」で通しておきます。この「審査済みのカード」があるかないかで、一番手争いの勝率が決まります。住宅ローン控除や補助金の出口戦略もここで確定させます。"
    },
    "3.物件調査": {
        "内容": "内見に加え、マンションなら『重要事項調査報告書』で積立金の不足や修繕計画を確認。戸建ならハザードマップや越境を確認します。",
        "アクション": "・昼夜、雨天時の周辺環境確認\n・ゴミ置き場の管理状態チェック\n・管理組合の財務状況確認",
        "ポイント": "【管理を買う】マンションは「管理を買え」と言われます。プロはエントランスではなく「駐輪場とゴミ置き場」を見ます。ここが乱れている物件は管理組合が機能しておらず、将来の修繕積立金不足や資産価値の下落を招きます。また、長期修繕計画書を読み込み、直近で「大規模修繕」があるか、その一時金徴収予定がないかを確認するのが必須のアクションです。戸建の場合は、接道や排水の権利関係を徹底的に深掘りします。"
    },
    "4.購入申込": {
        "内容": "買主としての意思表示を「買付証明書」にて行います。希望価格、支払方法、契約希望日、ローン特約の有無を記載します。",
        "アクション": "・買付証明書の記入・提出\n・手付金の準備（現金100〜数百万）",
        "ポイント": "【一番手の死守】不動産は早い者勝ちです。プロは内見
